...
void println(char *msg)
{
	...
	put_string(msg, strlen(msg), line, 0);
	line = (line + 1) % 25;
}
...
int load_kernel()
{
	int i, j, next_cluster, current_sector_num, 
		current_cluster_num, bytes_to_read, found = 0, load_done = 0;
	...
	fat.sector_reserved =
		get_le(fat.base_addr, SECTOR_RESERVED_OFFSET, 2);
	fat.sector_per_cluster =
		get_le(fat.base_addr, SECTOR_PER_CLUSTER_OFFSET, 1);
	...
	fat.root_cluster[fat.root_cluster_count++] 
		= get_le(fat.base_addr, ROOT_CLUSTER_OFFSET, 4);
	
	// Find all the clusters belonging to root directory
	next_cluster = get_le(
		fat.FAT_base + fat.root_cluster[0] / (SECTOR_SIZE / ADDRESS_SIZE),
		fat.root_cluster[0] % (SECTOR_SIZE / ADDRESS_SIZE) * ADDRESS_SIZE,
		ADDRESS_SIZE);
	...
	while (next_cluster != END_SIGNAL) {
		...
		next_cluster = get_le(
			fat.FAT_base
				+ next_cluster / (SECTOR_SIZE / ADDRESS_SIZE),
			next_cluster % (SECTOR_SIZE / ADDRESS_SIZE)
				* ADDRESS_SIZE,
			ADDRESS_SIZE);
	}
	...
	for (i = 0; i < fat.root_cluster_count; i++) {
		
		// for each sector in a cluster
		current_cluster_num = fat.root_cluster[i];
		for (j = 0; j < fat.sector_per_cluster; j++) {
			current_sector_num = fat.data_sector
				+ (current_cluster_num - BEGIN_CLUSTER)
				   * fat.sector_per_cluster
				+ j;
				
			read_sector(current_sector_data, current_sector_num, 0,
				SECTOR_SIZE);
				
			// for each sector in a cluster
			for (dentry = current_sector_data;
				dentry != current_sector_data + SECTOR_SIZE;
				dentry += 1) {
					
				if (dentry->attribute == ATTR_DISK
				    || dentry->attribute == ATTR_LONG_FILE_NAME
				...
				if (string_match(dentry->base_name,
						BASE_NAME_TO_FIND,
						BASE_NAME_TO_FIND_LENGTH)
					&& string_match(
						dentry->extended_name,
						EXTENDED_NAME_TO_FIND,
						EXTENDED_NAME_TO_FIND_LENGTH)
	...
	if (found) {
		...
		kernel_bin.data = LOAD_KERNEL_ADDRESS;
		kernel_bin.cluster_count = 0;
		kernel_bin.cluster[kernel_bin.cluster_count++] = 
			(dentry->cluster_high[0] << BITS_PER_BYTE * 2)
			+ (dentry->cluster_high[1] << BITS_PER_BYTE * 3)
			+ dentry->cluster_low[0]
			+ (dentry->cluster_low[1] << BITS_PER_BYTE);

		// Find clusters belonging to kernel.bin
		next_cluster = get_le(
			fat.FAT_base + kernel_bin.cluster[0] / (SECTOR_SIZE / ADDRESS_SIZE),
			kernel_bin.cluster[0] % (SECTOR_SIZE / ADDRESS_SIZE) * ADDRESS_SIZE,
			ADDRESS_SIZE);
		...
		for (i = 0; i < kernel_bin.cluster_count; i++) {
		    ...
		    bytes_to_read -= SECTOR_SIZE) {
				...
				if (bytes_to_read < SECTOR_SIZE) {
					read_sector(cursor, current_sector_num,
						    0, bytes_to_read);
					load_done = 1;
				}
				else
